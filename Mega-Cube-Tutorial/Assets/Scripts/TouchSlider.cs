...
public class TouchSlider : MonoBehaviour, IPointerDownHandler, IPointerUpHandler 
{
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        if (OnPointerDownEvent != null) 
            OnPointerDownEvent.Invoke();  //tek satýrlýk if'lerde curly bracket'e gerek yokmuþ.

        if (OnPointerDragEvent != null)
            OnPointerDragEvent.Invoke(uiSlider.value); // tek satýrlýk if
    }
    private void OnSliderValueChanged(float value)
    {
        if (OnPointerDragEvent != null) 
            OnPointerDragEvent.Invoke(value); // tek satýrlýk if
    }
